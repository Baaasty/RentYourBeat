<template>
  <div class="z-50 flex w-full justify-center">
    <header
      class="ml-6 mr-6 flex h-20 w-full items-center justify-between text-white lg:w-[85%] xl:w-[75%]"
    >
      ...
      <nav>
        <ul class="hidden space-x-4 lg:flex">
          <li v-for="link in links">
            <NuxtLink
              :to="link.path"
              class="delay transition-colors duration-150 ease-in-out hover:text-gray-300"
            >
              {{ link.name }}
            </NuxtLink>
          </li>
